...
func (zftf *ZipFunctionTemplateFetcher) Fetch() ([]*FunctionTemplate, error) {
	...
	var err error
	//zftf.logger.DebugWith("111111111111111111111111111111111")
	...
	for _, file := range files {
		zftf.logger.DebugWith("File in directory", "fileName", file.Name())
	}
	if common.IsLocalFileURL(zftf.fileAddress) {
		//localFilePath := common.GetPathFromLocalFileURL(zftf.fileAddress)
		localFilePath := "/mnt/templates.zip"
		zftf.logger.DebugWith( "localFilePath", localFilePath)
